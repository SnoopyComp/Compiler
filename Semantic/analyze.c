...
static void insertNode(TreeNode *t)
{
	switch (t->kind)
	{
		...
		case VarAccessExpr:
		{
			...
			SymbolRec *func = lookupSymbolWithKind(currentScope, t->name, FunctionSym);
			if(func == NULL) func = UndeclaredFunctionError(currentScope, t);
			// Update Symbol Table Entry
			else
...
static void checkNode(TreeNode *t)
{
	switch (t->kind)
	{
		...
		case WhileStmt:
		{
			...
			int kind = t->child[0]->kind;
			// Nodekind error
			if(kind != AssignExpr && kind != ConstExpr && kind != BinOpExpr && kind != VarAccessExpr ) InvalidConditionError(t->lineno);
			// Expr type error
			else
			{
				int type = t->child[0]->type;
				if(type == Void || type == VoidArray || type == IntegerArray) InvalidConditionError(t->lineno);
		...
		case ReturnStmt:
		{
			...
			if(currentScope->func->type != t->type)
		...
		case VarAccessExpr:
		{
			...
			if (t->child[0] != NULL)
			{
				...
				else if(t->child[0]->type != Integer && t->child[0]->kind != ConstExpr) ArrayIndexingError(t->name, t->lineno);
				/*********************Fill the Code*************************
				 *                                                         *
				 *                                                         *
				 *                                                         *
