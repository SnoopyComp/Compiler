...
fun_declaration     : type_specifier identifier LPAREN params RPAREN compound_stmt
                         { 
						$$ = newTreeNode(FunctionDecl); $$->lineno = $2->lineno; $$->type = $1->type;
                              $$->name = $2->name;
                              $$->child[0] = $4; $$->child[1]=$6;
                              free($1); free($2);
                         }
...
params              : param_list { $$ = $1; }
                    | VOID
                         {
						$$=newTreeNode(Params); $$->lineno=lineno; $$->flag=TRUE;
                         }
                    ;
...
param               : type_specifier identifier
                         {
						$$ = newTreeNode(Params); $$->lineno = $2->lineno; $$->type = $1->type;
                              $$->name = $2->name;
                              free($1); free($2);
                         }
                    | type_specifier identifier LBRACE RBRACE
                         { 
						$$ = newTreeNode(Params); $$->lineno = $2->lineno;
                              if ($1->type == Void)    $$->type = VoidArray;
                              else if ($1->type == Integer)    $$->type = IntegerArray;
                              else $$->type = None;
                              $$->name = $2->name;
                              free($1); free($2);
                         }
...
compound_stmt       : LCURLY local_declarations statement_list RCURLY 
						...
						$$ = newTreeNode(CompoundStmt); 
                              if($2==NULL) $$->child[0] = $3;
                              else {$$->child[0] = $2; $$->child[1] = $3;}
                              if($3!= NULL) $$->type = $3->type;
                         }
                    ;
...
selection_stmt		: IF LPAREN expression RPAREN statement ELSE statement
						{
							$$=newTreeNode(IfStmt); $$->lineno=$3->lineno;
                                   $$->child[0] = $3; $$->child[1] = $5; $$->child[2]=$7;
                                   $$->flag=TRUE;
						}
...
iteration_stmt      : WHILE LPAREN expression RPAREN statement
                         { 
						$$=newTreeNode(WhileStmt); $$->lineno=$3->lineno;
                              $$->child[0] = $3; $$->child[1] = $5;
                         }
                    ;
return_stmt         : RETURN SEMI 
						{ 
							$$=newTreeNode(ReturnStmt); $$->flag=TRUE;
						}
                    | RETURN expression SEMI
                         { 
                              $$=newTreeNode(ReturnStmt); $$->flag=0;
                              $$->child[0] = $2; $$->type = $2->type;
                         }
                    ;
expression          : var ASSIGN expression
                         { 
                              $$=newTreeNode(AssignExpr); $$->lineno = $1->lineno;
                              $$->child[0] = $1; $$->child[1] = $3;
                         }
                    | simple_expression { $$ = $1; }
                    ;
...
call                : identifier LPAREN args RPAREN
                         { 
						$$=newTreeNode(CallExpr); $$->lineno = $1->lineno;
                              $$->name = $1->name; 
                              $$->child[0] = $3;
                              free($1);
                         }
